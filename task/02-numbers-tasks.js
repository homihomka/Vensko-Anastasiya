...
function getAngleBetweenVectors(x1, y1, x2, y2) {
    ...
    const angle = scalarVectors / (xVector * yVector);
...
function roundToPowerOfTen(num, pow) {
    const splitter = 10 ** pow;
    const rounded = Math.round(num / splitter);
    return splitter * rounded;
}
...
function toNumber(value, def) {
    const isNumber = (typeof value === 'number' || value instanceof Number || Number(value));
    return (isNumber ? value : def)
}
